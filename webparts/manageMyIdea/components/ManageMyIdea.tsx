...
import styles from './ManageMyIdea.module.scss';
import { IManageMyIdeaProps } from './IManageMyIdeaProps';
...
export default class ManageMyIdea extends React.Component<IManageMyIdeaProps, ManageStates> {
  ...
  public render(): React.ReactElement<IManageMyIdeaProps> {
    ...
    return (
      <React.Fragment>
        <InitialLoad />
        <Container className={styles.managemyidea + " nopadding"} fluid={true}>
            ...
            <Row className={styles.mdmargintop}>
              <Col xs={12} md={12} lg={12} className={styles.bgcolor}>
                <Row className={styles.IdeaNav}>
                  ...
                  <Col xs={5} md={5} lg={5} className={styles.breadcrumb}>MY IDEAS</Col>
                ...
                <div>
                  <Slider {...settings}>
                    {this.state.filteredTask.length > 0 ? this.state.filteredTask.map(
                      (item, key) => {
                        let navDetailsURL = sitePagesURL + "/IdeaDetails.aspx?iid=" + item.ID + "&pid=" + item.PhaseId + "&idp=mng";
                        let bgimageURL = "";
                          ...
                          <Col xs={12} md={12} lg={12} className={styles.slidercontainer}>
                            <Row className={styles.carddisplay}>
                              ...
                              {<Col xs={8} md={8} lg={8} >
                                ...
                                {/* <a className={styles.cardlink} onClick={(e) => { this.setIdeaPhasePage(item.ID, item.PhaseId, "mng"); }} title={item.IdeaTitle}><h3>{truncatorheader(item.IdeaTitle)}</h3></a> */}
  ...
  public GetMyIdeas() {
    sp.web.lists.getByTitle("Ideas").items.top(4999).select("*, Phase/Title,Author/Title,Influencers/Title").expand("Phase, Author,Influencers").filter('Author%20eq%20' + this.state.CurrentUserID + 'or Influencers%20eq%20' + this.state.CurrentUserID)
      .orderBy('Created')
      .get().then((item: any) => {
